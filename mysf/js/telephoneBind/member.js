...
function createNewDom(json) {
	...
	"<li class='item item-fm-m'><div class='ui-div-right' style='padding: 5px 0; text-align:right'> <a class='ui-btn btn-delete-line'  href='#' id='deleteItem'>删除运单</a></div></li>" +
	"</ul> </form>"+
		...
		if("80"==$.trim(item.opCode) || "8000"==$.trim(item.opCode)){
			...
			if($.trim(item.routeJson)!=''){//组合路由链表
				...
				/*if( "8000"==$.trim(item.opCode)){
					...
					//type=2&json={"fromCityName":"深圳","toCityName":"广州","waybillno":"939659732549","signTime":"2015-08-17%2015:53:53","employ":{}}
					var evaluateLink="/page/telephoneBind/deliverydetails/routing/evaluate.html?json="+item.routeJson;
...
function myExpressForward(orderNo,opCode,type){
		// 跳转到详细页面
		var url = "/page/telephoneBind/deliverydetails/routing/";
		/**if (opCode == "50") {
...
function loadOrderList(orderList){
	...
	var sindingOrder=" <section class='grid-main' style='margin-top: 10px;'><form><ul class='ui-fm'><li class='item item-fm-m'><div class='ui-div-right' style='padding: 5px 0;' >" +
			...
			"<div class='ui-div-right' style='padding: 5px 0;' ><span>订单号：<span name='ydNo' id='orderNo'></span></span></div></a></li></ul></form></section>";
	//已完成订单
	...
	"<a href='#' class='ui-btn-right3' ><div class='ui-div-right' style='padding: 5px 0;color: rgb(172,172,172);' ><span>订单号：<span name='ydNo'  id='orderNo'></span></span></div></a></li></ul></form> <div class='box-yc'><img src='../../../css/common/img/yc_icon.png'></div><div class='clearall'></div></section>";
	//取消订单
	...
	$.each(orderList,function(index,orderInfo){
		...
		if(orderInfo.orderstatus == "0" 
			...
			if(sentWay=='2'){
				...
				hkOrderTempObj.find("#forwardUrl").attr("href","/page/telephoneBind/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
				...
				orderTempObj.find("#forwardUrl").attr("href","/page/telephoneBind/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
			...
			if(opCode=="80" || opCode=="8000"){
				...
				completeOrderObj.find("#forwardUrl").attr("href","/page/telephoneBind/deliverydetails/routing/waybill-4-1-routing.html?bno="+bno+"&hasRoute=true").removeAttr("id");
				completeOrderObj.find("#deleteItem").attr("onClick","deleteMyExpress('"+orderNo+"','"+2+"')").removeAttr("id");
				//怎加完成订单后点击订单链接
				completeOrderObj.find(".ui-btn-right3").attr("href","/page/telephoneBind/orderlist/orderDetails.html?bno="+orderNo);
				...
				sindingOrderObj.find(".ui-btn-right3").attr("href","/page/telephoneBind/orderlist/orderDetails.html?bno="+orderNo);
				//如果预计送达时间为空
				if($.trim(orderInfo.expectedDate)==""){
			...
			cancelOrderObj.find("#forwardUrl").attr("href","/page/telephoneBind/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
