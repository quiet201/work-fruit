...
$(document).ready(function(){
	...
	$.post("/service/commonLog/addLog/L00201");
		...
		//var url="/../../telephoneBind/deliverydetails/no-routing/sf07_1-1-ok.html?bno="+bno;
...
function getData(ruleConfig,obj) {
	// 数据提交
			$.ajax({
				...
				dataType : "json",
				url : "/service/delivery/arrive/query44BarWayBill/" + obj.bno+"/false",
						...
						if (values) {
							...
							if (values.employ) {
								...
								var imgUrl = values.employ.photoUrl ? values.employ.photoUrl
										: "/image/telephoneBind/defalut-image.png";
								//如果图片不存在后缀，需要加上后缀
...
function btnClick(){
	...
	if('false'== noMessage){
		...
		$.ajax({
			...
			async : false,
			dataType : "json",
			url : "/service/order/orderContro/aMessage/"+bno+"/1/"+selectedLabel+"/"+encodeURI(encodeURI(msg)),
			success : function(data) {
				if(null!=data){
				//var url="/../../telephoneBind/deliverydetails/no-routing/sf07_1-1-ok.html?bno="+bno;	
		...
		$('#subBtn').attr('class','ui-btn btn-submit-no');
		$('#subBtn').attr('onclick','');
		}
	}
}
