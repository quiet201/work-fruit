...
function init(){
	queryAjax();
	url=getUrlValueByKey("addressToPage");
	
	//改变地址栏参数
	if(url!=null&&url!="" && typeof(url)!='object'){
		  setCookie('addressToPage',url);
		}else{
		  url = getCookie('addressToPage');	
	}
}
...
function queryAjax() {
	...
	$.ajax({ 
		...
		success : function(data) {
			$("#waitplease1").hide();
			if (data) {
				var temp="<li class='item item-fm-m' node='selectItemForward' ><div class='fill-more div-padding-top-buttom10'><span class='lx-add'><p><span class='name text-fontStyle'></span>"+
				...
				"</a>选择此地址</div></div><div class='' style='padding-top: 13px;'><a id='editAddress' class='btn-buttom-line btn-color-hui' href='#'>编辑</a><a id='cancelItem' class='btn-buttom-line btn-color-hui' href='#' style='margin-left: 10px;'>删除</a>"+
				"</div> </div> </li>";
				...
				for(var i=0;i<data.length;i++){
					...
					if(data[i].addressStatus == 1){
						//tempObject.find("#defaultSet").removeClass().addClass("ui-ico-sfi ico-circle-tick ico-ck-style").removeAttr("id");
						tempObject.find(".address").html("<p class='defort_lab'>【默认】</p>"+data[i].areaName+data[i].detailAdress);
...
//				alert("设为默认地址成功");
				if($.trim(data)!="N"){
					...
					if(!isNotForward){
						...
						location.href = url+"?asbkId="+asbkId;
					}
...
function editAddressInfo(asbkid){
	...
	location.href = "/page/telephoneBind/addressbook/send/address_edit_sent1.html?asbkId="+asbkid;
};
//删除地址簿
...
//						alert("删除成功!");
						if($.trim(data)!=""){
							...
							setTimeout(function() {
								location.href = "/page/telephoneBind/addressbook/send/address_menber1.html";
							},1000);
						}else{
							tipsDialog("删除失败");
							console.log("删除失败...");
...
function checkOne(index){
	...
	setTimeout(function() {
		...
		location.href = url+"?asbkId="+index;
						...
						success : function(dataOne) {
							if(dataOne){
								...
								setTimeout(function() {
									var tempUrl=url;
									if($.trim(url)==""){
										 tempUrl= getCookie('addressToPage');
										
									}
									location.href= tempUrl+"?asbkId="+dataOne.asbkId;;
								},80);
							}else{
								// 数据提交
								$.ajax({
												...
												success : function(data) {
													if(data){ 
														setTimeout(function() {
															...
															location.href= tempUrl+"?asbkId="+data.asbkId;
															//window.location.href = "ship.html?asbkId="+data.asbkId;
														},80);
													}
												},
												error : function(e) {
