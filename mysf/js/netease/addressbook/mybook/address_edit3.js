...
function listem(){
	$("#checkedOne").unbind().bind("click",function(){
		...
		getAddressDetail("/page/netease/address/province.html?addrName=destName&addrCode=destCode&addrCountyName=dcountyName&addrCountyCode=dcountyCode&type=3&gatChild=-1");
	});
		...
		$.ajax({
			...
			success : function(data) {
				if($.trim(data)!="N"){
					...
					setTimeout(location.href = "/page/netease/addressbook/mybook/address_menber3.html",80);
				}else{
