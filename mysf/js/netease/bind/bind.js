...
function dosubmit(){
	...
	$.ajax({
		...
		data : mm,
		dataType : "json",
		url : "/service/netease/buttonToPhonebind",
		success: function(code){
			if(code+"" == "200"){
				var temp=phoneNo.substring(0,3)+"****"+phoneNo.substring(phoneNo.length-4,phoneNo.length);
				window.location = "/page/netease/bind/binded.html?phone="+temp;
			}else{
...
function sendValidateCode(){
	...
	var url = "/service/netease/phonebindcode/"+$("#phoneNo").val();
	$.ajax({
		type : "GET",
		dataType : "json",
		url : url,
