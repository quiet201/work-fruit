 ...
 //判断当前运单状态 1。电子运单  0.普通运单 默认为电子运单
    ...
    //var url ='../../netease/range/ship_submit.html';
		...
		$.ajax({
			...
			success: function(json){
				if(json){
					...
					window.location = "/../../netease/range/ship_submit.html?orderJson="+encodeURI(json);
				}
			},
			error:function(XMLHttpRequest, textStatus, errorThrown){
	...
	function toConfirmPage(){
		var orderJson =  packagingOrderData();
		window.location = "/../../netease/range/ship_submit.html?orderJson="+encodeURI(orderJson);
	}
	...
	function toSelectAddressBook(){
		...
		location.href = "/page/netease/addressbook/send/address_menber1.html?addressToPage=/page/netease/order/ship.html&shipAskId="+shipAskId;
	}
...
//			       "<tr class='tr-bottom'><td colspan='3'><p>"+'全国及港澳台高标准的门到门快件服务'+"</p></td></tr>";
	...
	function submitOrder(){
		...
		if(checkAddressLength($("#senderAddress").text(),"亲，寄件人地址不能超过50个字哦~")&&
				checkAddressLength($("#shippingAddress").text(),"亲，收件人地址不能超过50个字哦~")){
		$.ajax({
			type : "POST",
			...
			success: function(result){
				if(result){
					if(result.code == "200"){
						...
						clearCookie();
						//跳转成功页面
						window.location = "/page/netease/order/ship_success.html?resultJson="+JSON.stringify(result);
...
//		$(".Amount").val($(".Amount").val().replace(/[^\d]/g,''));
	...
	function checkAddressLength(text,content){
