...
//		data : {asbkId:asbkid},
//		dataType : "json",
//		url : "/service/sendaddress/sendbook/findAskbCount/"+asbkid,
//		success : function(data) {
//			if(data >= 1){
//				tipsDialog("该地址已被引用,不可被删除");
//			}else{
				$.ajax({
					type : "POST",
					data : {asbkId:asbkid},
...
//						alert("删除成功!");
							...
							setTimeout(function() {
								window.location.href = "address_menber.html";
							},80);
						}else{
							tipsDialog("删除失败");
							console.log("删除失败...");
	...
	$.ajax({
		...
		success : function(data) {
			if (data) {
				...
				$("#editeAddress").html('<a class="ui-btn btn-top" href="address_medite.html?asbkId='+data.asbkId+'"title="编辑" onclick="commonEventPush(\''+"寄件"+'\''+","+'\''+"页面统计"+'\''+","+'\''+"编辑详情"+'\''+","+'\''+"address_info.html"+'\')">编辑</a>');
			}else{
				//tipsDialog("查询失败");
