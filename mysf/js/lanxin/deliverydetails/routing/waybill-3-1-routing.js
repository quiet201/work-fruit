						...
						if('true'==isMessage){
							...
							if('1'==isBind){///如果是本人
								...
								if('true'==doBookSendTime){
									...
									$("#bookTime").attr("href","/page/lanxin/deliverydetails/bookTime/bookSendTime.html?bno="+bno);
								}
								else if('false'==doBookSendTime){
									$('#bookTime').attr('class','ui-btn btn-submitserver');
									$("#bookTime").text("查看预约派件");
									$("#bookTime").attr("href","/page/lanxin/deliverydetails/bookTime/bookSendTime_success.html?bno="+bno);
								}
							}
							else if('1'!=isBind){
			...
			////////////预约派件时间/////////
						...
						if (values) {
							...
							if (values.employ) {
								...
								var xiaogeDs="/page/lanxin/accepted/accepted.html?empName="+deliverName+"&mobile="+
