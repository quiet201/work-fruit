...
function createNode(hotList,provinceList,countryList, gatList) {
	...
	$.each(hotList, function(index, data) {
		...
		hostBuff = hostBuff + "<li><a href='/page/lanxin/address/county.html?city="
	...
	$.each(provinceList, function(index, data) {
		...
		proBuff = proBuff + "<li><a href='/page/lanxin/address/city.html?pro=" +data.distCityCode
	...
	$.each(gatList, function(index, data) {
		/*gatBf = gatBf + "<li><a href='/lanxin/range/city.html?pro=" +data.distCityCode
	...
	/*$.each(countryList, function(index, data) {  //如果以后有数据 ，国际信息可以直接获取到市，区，可以用此来处理
		...
		//countryBf = countryBf + "<li><a href='/lanxin/range/city.html?pro=" +data.distCityCode
		//+ "," + data.distCnName + ","+ data.distCode+"'>" + data.distCnName + "</a></li>";
	});*/
