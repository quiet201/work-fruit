		...
		if("80"==$.trim(item.opCode) || "8000"==$.trim(item.opCode)){
			...
			if($.trim(item.routeJson)!=''){//组合路由链表
				...
				/*if( "8000"==$.trim(item.opCode)){
					...
					//type=2&json={"fromCityName":"深圳","toCityName":"广州","waybillno":"939659732549","signTime":"2015-08-17%2015:53:53","employ":{}}
					var evaluateLink="/page/lanxin/deliverydetails/routing/evaluate.html?json="+item.routeJson;
...
function myExpressForward(orderNo,opCode,type){
		// 跳转到详细页面
		var url = "/page/lanxin/deliverydetails/routing/";
		/**if (opCode == "50") {
...
//已完成订单
	...
	var evaluateSend = "/page/lanxin/deliverydetails/routing/evaluateSend.html";
	$.each(orderList,function(index,orderInfo){
		...
		if(orderInfo.orderstatus == "0" 
			...
			if(sentWay=='2'){
				...
				hkOrderTempObj.find("#forwardUrl").attr("href","/page/lanxin/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
				...
				orderTempObj.find("#forwardUrl").attr("href","/page/lanxin/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
			...
			if(opCode=="80" || opCode=="8000"){
				...
				completeOrderObj.find("#forwardUrl").attr("href","/page/lanxin/deliverydetails/routing/waybill-4-1-routing.html?bno="+bno+"&hasRoute=true").removeAttr("id");
				...
				//怎加完成订单后点击订单链接
				completeOrderObj.find(".ui-btn-right3").attr("href","/page/lanxin/orderlist/orderDetails.html?bno="+orderNo);
				...
				sindingOrderObj.find(".ui-btn-right3").attr("href","/page/lanxin/orderlist/orderDetails.html?bno="+orderNo);
				//如果预计送达时间为空
				if($.trim(orderInfo.expectedDate)==""){
			...
			cancelOrderObj.find("#forwardUrl").attr("href","/page/lanxin/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
