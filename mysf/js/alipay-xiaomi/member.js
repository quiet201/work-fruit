...
$(document).ready(function(){
	...
	//加载公共资源,并执行初始化方法
	getDescPage("commonPage","/page/common/tips/tips.html",'getUser');
	
});
function getUser(){
	var uid=getUrlValueByKey("uid");
	if($.trim(uid)!=""){
		$.ajax({
			type : "post",
			dataType : "json",
			data:{"uid":uid},
			url : "/service/xiaomi/loginActivity",
			success : function(json) {
				//如果已经设置好session，则调用下面
				if(json || $.trim(json.result)==="Y"){
					console.info("ok");
				}
				init();
			}, 
		...
		if("80"==$.trim(item.opCode) || "8000"==$.trim(item.opCode)){
			...
			if($.trim(item.routeJson)!=''){//组合路由链表
				...
				/*if( "8000"==$.trim(item.opCode)){
					...
					//type=2&json={"fromCityName":"深圳","toCityName":"广州","waybillno":"939659732549","signTime":"2015-08-17%2015:53:53","employ":{}}
					var evaluateLink="/page/xiaomi/deliverydetails/routing/evaluate.html?json="+item.routeJson;
...
function myExpressForward(orderNo,opCode,type){
		// 跳转到详细页面
		var url = "/page/xiaomi/deliverydetails/routing/";
		/**if (opCode == "50") {
...
function loadOrderList(orderList){
	...
	var evaluateSend = "/page/xiaomi/deliverydetails/routing/evaluateSend.html";
	$.each(orderList,function(index,orderInfo){
		...
		if(orderInfo.orderstatus == "0" 
			...
			if(sentWay=='2'){
				...
				hkOrderTempObj.find("#forwardUrl").attr("href","/page/xiaomi/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
				...
				orderTempObj.find("#forwardUrl").attr("href","/page/xiaomi/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
			...
			if(opCode=="80" || opCode=="8000"){
				...
				completeOrderObj.find("#forwardUrl").attr("href","/page/xiaomi/deliverydetails/routing/waybill-4-1-routing.html?bno="+bno+"&hasRoute=true").removeAttr("id");
				...
				//增加a标记的点击链接
				completeOrderObj.find(".ui-btn-right3").attr("href","/page/xiaomi/orderlist/orderDetails.html?bno="+orderNo);
				...
				sindingOrderObj.find(".ui-btn-right3").attr("href","/page/xiaomi/orderlist/orderDetails.html?bno="+orderNo);
				//如果预计送达时间为空
				if($.trim(orderInfo.expectedDate)==""){
			...
			cancelOrderObj.find("#forwardUrl").attr("href","/page/xiaomi/orderlist/orderDetails.html?bno="+orderNo).removeAttr("id");
...
function showTip3(index,type){
	var $mask = $('<div class="maskbox" style="z-index:1006; opacity:.9;"></div>');
	$mask.append($(".tip-box"));
	$("body").append($mask);
	$("#tipsMaskbox").show();
