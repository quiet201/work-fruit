...
function listem(){
	$("#checkedOne").unbind().bind("click",function(){
		...
		getAddressDetail("/page/xiaomi/address/province.html?addrName=destName&addrCode=destCode&addrCountyName=dcountyName&addrCountyCode=dcountyCode&type=3&gatChild=-1");
	});
...
function updateAjax() {
	if(checkPhone()){
		...
		if(checkAddressLength($("#provinceName").val()+$("#cityName").val()+$("#countyName").val(),"亲，地址不能超过50个字哦~",150)){
		// 数据提交
		$.ajax({
			type : "POST",
			data : addressEntity,
			...
			success : function(data) {
				if($.trim(data)!="N"){
					...
					setTimeout(location.href = "/page/xiaomi/addressbook/mybook/address_menber3.html",80);
				}else{
