		...
		success : function(data) {
			...
			if (data.state == "Y") {
				...
				$("#evaluateItem").unbind().bind("click",function(){
					...
					location.href="/page/noUserFunction/deliverydetails/routing/evaluate.html?type="+tpye+"&json="+json;
				});
			}
		},
		error : function(e) {
	...
	$.ajax({
				...
				success : function(data) {
					...
					if ($.trim(data) != "") {
						...
						//设置跳转链接
						$(".ui-angle-right").attr("href","/page/noUserFunction/deliverydetails/routing/routeDetail.html?details="+jsonDetails);
						//小哥详情
						if (values.employ) {
							// 派送员手机
...
/*		location.href="/page/alipay/deliverydetails/routing/evaluate.html?json="+JSON.stringify(evaluateParams);;*/
	}
};
