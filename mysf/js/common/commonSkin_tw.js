...
function mem_confirm(_htmlTxt, _funcitonYes, _functionNo, _textYes, _textNo) {
	...
	var htmlButton = "<a class='btn-action' onclick='mem_callback_yes();' href='javascript:void(0)'>"
			+ textYes
			+ "</a><a onclick='mem_callback_no();' class='btn-action ml20' href='javascript:void(0)'>"
			+ textNo + "</a>";
...
function mem_confirm2(_htmlTxt, _funcitonYes, _textYes) {
	...
	var htmlButton = "<a class='btn-action' style='height:30px;' onclick='mem_callback_yes();' href='javascript:void(0)'>"
			+ textYes + "</a>";
	dialog(htmlTxt, htmlButton, 1);
}
...
function isMobel(value) {
	if (/^13\d{9}$/g.test(value) || /^14[5,7]\d{8}$/g.test(value)
	...
	getCarBrand : function(data) {
		...
		if (deptList) {
			for ( var key in deptList) {
				count++;
				if (count % 4 == 0) {
					option += "&nbsp;<input type='radio' name='deptCode' value='"
							+ deptList[key].deptId
							+ "'/>"
							+ deptList[key].deptName + "<br>";
				} else {
					option += "&nbsp;<input type='radio' name='deptCode' value='"
							+ deptList[key].deptId
							+ "'/>"
							+ deptList[key].deptName;
				}
			}
	...
	getChannList : function(data) {
		...
		if (infoList) {
			for ( var key in infoList) {
				option += "<option value='" + infoList[key].code + "'>"
						+ infoList[key].chlName + "</option>";
			}
