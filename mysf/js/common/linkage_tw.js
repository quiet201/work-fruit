...
function rootClick(){
	...
	'<div id="hot" style="display: none;" class="topPanel toDelete">'+
		...
		'<section class="mainContent">'+	
		    ...
		    '<ul class="city-list">'+
		    	'<li><a href="javascript:void(0)" onclick="hotTopr()">選擇省份</a></li>'+            
		    '</ul>'+
		    '</section>'+
		'</section>'+
	'</div>'+
...
function saveInfo_s(callback){
    ...
    if(!/^1[0-9]{10}$/.test(user_pno)
    	...
    	alert("請填寫正確的手機或固話號碼,固話號碼的格式為:區號-號碼-分機號");
		return false;
	}
...
function getLocation() {
	...
	if (navigator.geolocation) {
		//若支持地理位置获取,成功调用showPosition(),失敗调用showError
		dialog("正在努力獲取位置...");
