...
function createNode(cityList,proInfo){
		...
		$.each(cityList,function(index,data){
			cityBuff=cityBuff+"<li><a href='/page/common/address/county.html?city="+data.distCityCode+","+data.distCnName+","+data.distCode+"&pro="+proInfo+"'>"+data.distCnName+"</a></li>";
		});
		$("#cityList").empty().html(cityBuff);
};
function redirectProvice(){
	var interType=getCookie("interType");
	location.href="/page/common/address/province.html?type="+interType;
};
