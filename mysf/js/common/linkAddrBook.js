 ...
 * 设置地址数据
 */
function setInfo1(json,addressType){
	//设置默认地址
	if(addressType == 'r'){
		if(json){
...
function setInfo(json,addressType){
	...
	if(isMobel(mobile)){
		$("#raddr").val(json.fullname+"  "+json.mobile);
		$("#raddrDetail").val(json.provinceName +  json.cityName + json.countyName+"  "+json.address);
		$("#raddrId").val(json.userAddressId);  //ADDRESS_ID
	}else{
		$("#raddr").val(json.fullname+"  "+json.phone);
		$("#raddrDetail").val(json.provinceName +  json.cityName + json.countyName+"  "+json.address);
		$("#raddrId").val(json.userAddressId);
	}	
	//$("#user_address").val(json.address);
	//$("#user_show_all").val(json.provinceName + " " + json.cityName + " " + json.countyName);
 ...
 * 设置地址数据
 */
function setInfo3(json,addressType){
	//设置默认地址
	if(addressType == 'r'){
		if(json){
			weixin_order_pram.useAddress_r = json;
	...
	if(isMobel(mobile)){
		$("#raddr").html(json.fullname+"  "+json.mobile);
		$("#raddrDetail").html(json.provinceName +  json.cityName + json.countyName+"  "+json.address);
		$("#raddrId").val(json.userAddressId);  //ADDRESS_ID
	}else{
		$("#raddr").val(json.fullname+"  "+json.phone);
		$("#raddrDetail").val(json.provinceName +  json.cityName + json.countyName+"  "+json.address);
