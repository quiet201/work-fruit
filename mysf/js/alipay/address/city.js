...
function createNode(cityList,proInfo){
		...
		$.each(cityList,function(index,data){
			cityBuff=cityBuff+"<li><a href='/page/alipay/address/county.html?city="+data.distCityCode+","+data.distCnName+","+data.distCode+"&pro="+proInfo+"'>"+data.distCnName+"</a></li>";
		});
		$("#cityList").empty().html(cityBuff);
};
function redirectProvice(){
	var interType=getCookie("interType");
	location.href="/page/alipay/address/province.html?type="+interType;
};
