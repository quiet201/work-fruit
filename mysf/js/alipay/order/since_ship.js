 ...
 //判断当前运单状态 1。电子运单  0.普通运单 默认为电子运单
    ...
    //var url ='../../alipay/range/ship_submit.html';
...
//		 }
		...
		$.ajax({
			...
			success: function(json){
				if(json){
					...
					window.location = "/../../alipay/range/ship_submit.html?orderJson="+encodeURI(json);
				}
			},
			error:function(XMLHttpRequest, textStatus, errorThrown){
	...
	function toConfirmPage(){
		var orderJson =  packagingOrderData();
		window.location = "/../../alipay/range/ship_submit.html?orderJson="+encodeURI(orderJson);
	}
...
//		}
		...
		if(isElectronicAirWaybill==1){
	        ...
	        var prodType = $("#input_id_productType").val();
		    orderJSON = '{'
			+'"orders":{'
			+'"weight":"'+ weight +'",'						//重量
...
//		cityNames[3] = ['755'];
	...
	function toSelectAddressBook(){
		...
		location.href = "/page/alipay/addressbook/send/address_menber1.html?addressToPage=/page/alipay/sinceorder/ship.html&shipAskId="+shipAskId;
	}
			...
			success: function(result){
				if(result){
					if(result.code == "200"){
						...
						clearCookie();
						//跳转成功页面
						window.location = "/page/alipay/sinceorder/ship-store-success.html?resultJson="+JSON.stringify(result);
...
function selectedProductClick(productType, productName, priceAndUnit){
	$("#input_id_productType").val(productType);
	$("#js_content_cplx").text(productName);
...
//            //生成时间列表
//            if(dateList != null && dateList.length == 3){
//                var dateUlContent = '<li>';  
//                dateUlContent += '<span value=' + dateList[0] +'>今天('+dateList[0]+')</span>';  
//                dateUlContent += '<ul> ';   
//                var todayTimeList = data.todayTimeList;
//                if(todayTimeList != null && todayTimeList.length > 0){
//                    for(var i=0; i< todayTimeList.length; i++){
//                            dateUlContent +=   '<li>'+ todayTimeList[i] + '</li> ';   
//                    }
//                }
//                dateUlContent +=   '</ul>';     
//                dateUlContent +=   '</li>';  
//                
//                dateUlContent += '<span value=' + dateList[1] +'>明天('+dateList[1]+')</span>'; 
//                dateUlContent += '<ul> ';   
//                var start =  dateList[1] +" " +data.orderStartTime+":00";
//                var startTm= new Date(start.replace(/-/g,"/")) ;
//                var end =  dateList[1] +" " +data.orderEndTime+":00";
//                var endTm= new Date(end.replace(/-/g,"/")) ;
//                var b = 30; //分钟数
//                while(endTm.getTime()-startTm.getTime()>=0){
//                        dateUlContent +=   '<li value='+startTm.Format("yyyy-MM-dd HH24:mm")+'>'+ startTm.Format("HH24:mm")+ '</li> ';   
//                        startTm.setMinutes(startTm.getMinutes() + b, startTm.getSeconds(), 0);
//                    }
//                dateUlContent +=   '</ul>';     
//                dateUlContent +=   '</li>'; 
//                
//                
//                
//                dateUlContent += '<span value=' + dateList[2] +'>后天('+dateList[2]+')</span>'; 
//                dateUlContent += '<ul> ';   
//                var start =  dateList[2] +" " +data.orderStartTime+":00";
//                var startTm= new Date(start.replace(/-/g,"/")) ;
//                var end =  dateList[2] +" " +data.orderEndTime+":00";
//                var endTm= new Date(end.replace(/-/g,"/")) ;
//                while(endTm.getTime()-startTm.getTime()>=0){
//                    dateUlContent +=   '<li value='+startTm.Format("yyyy-MM-dd HH24:mm")+'>'+ startTm.Format("HH24:mm")+ '</li> ';   
//                    startTm.setMinutes(startTm.getMinutes() + b, startTm.getSeconds(), 0);
...
//                return $('#treelist>li ul li').eq(array[1]).val()/* .trim(' ') */;  
//            }  
//    });  
//
//    $("#js_content_jjsj").bind('click',function(){
//    	var senderCityCode = $("#senderCityCode").val();
//		if(!senderCityCode || senderCityCode == null || $.trim(senderCityCode) == ""){
//			tipsDialog("请选择寄件人信息");
////			return false;
