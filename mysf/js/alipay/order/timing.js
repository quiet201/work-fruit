$(document).ready(function(){
	...
	queryDto={"srcAddr":"",//原寄地市编码
			...
			"srcCountDistCode":"",//原寄地地址代码
			"destCountDistCode":""//目的地地址代码
	}; 
	//加载公共资源,并执行初始化方法
...
function init(){
	...
	var countDistCode = getUrlValueByKey("countDistCode");
	//如果获取的是原寄地信息
	...
	if(srcCode){
		...
		setDtoParam(queryParam,"srcCountDistCode",countDistCode);
	...
	if(destCode){
		...
		setDtoParam(queryParam,"destCountDistCode",countDistCode);
		var dtoJson=JSON.stringify(queryParam);
		//setCookie("queryDto",dtoJson);
		setCookieTime('queryDto',dtoJson);
	}
...
function addListen(){
	...
	$(".ui-btn.btn-buttom-line3").unbind().bind("click",function(){
		location.href="/page/alipay/order/ship.html";
	});
};
//校验查件信息
