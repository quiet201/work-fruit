	...
	$.ajax({
				...
				success : function(data) {
					...
					if ($.trim(data) != "") {
						...
						getSubscription(obj.bno,"subscreptionButton","btn-submitserver-no","btn-submitserver",isMy.IsBind=="1","1");//订阅服务
						if (values) {
							//1为本人，0为非本人
							if(isMy.IsBind=="1"){
								//订阅坚挺subscreptionBno
...
function createTwoImage(bnos){
	...
	$.ajax({
		...
		success : function(data) {
			...
			//将此路径转发至安全签收页面
			var imgPath = data;
			var url = "/page/common/qrcode/qrcode_alipay.html";
