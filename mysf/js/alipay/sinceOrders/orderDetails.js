...
function fullInfo(result){
	...
	if(null!=result.consigneeAddress&&''!=$.trim(result.consigneeAddress)){
		...
		$('#sentAddress').text(result.consigneeAddress);	
			...
			var days = getDateDiff(result.orderDate,nowDate);	
			if(1<days){
				//此时订单失效
				$("#orderStatus").html("已失效");
				$("#status_effective").hide();
				$("#status_invalid").show();
			}else{
				$("#orderStatus").html("等待打印");
				$("#status_effective").show();
				$("#status_invalid").hide();
				//为打印订单绑定打印事件
		
				$("#dyOrder").attr("href","/page/alipay/sinceorder/ship-store-print.html?orderNo="+result.printOrderNo);
			}
...
function getUrlParams(urlParams) {
	...
	if (url.indexOf("?") != -1) {
		...
		for (var i = 0; i < strs.length; i++) {
			...
			/*if ([ strs[i].split("=")[0] ] == "status") {
				urlParams.status = unescape($.trim(strs[i].split("=")[1]));
			}*/
		}
	}
}
...
function getData(result){	
	...
	$.ajax({
		type : "get",
		dataType : "json",
		url : "/service/order/getMyOrderByBno/"+result.bno,
		async: false,
...
function hurryToSend(e){
	//var orderNo='C000000413';	
		$.ajax({
			...
			data : null,
			dataType : "json",
			url : "/service/order/orderContro/changeOrder/"+bno+"/1",
		...
		success: function(json){
			if(json && json != null){
				if(json.code == "200"){
					...
					//("订单取消中，已提交系统处理");
					//window.location.href="/page/alipay/orderlist/orderDetails.html?bno="+orderNo;
