...
function fullInfo(params){
	...
	if("0"==params.status){
		...
		$("#order_success").hide();
		//给重新打印按钮添加事件
		$("#goPrint").attr("onclick","printAgain('"+params.bno+"')");
	}else{
		$("#order_error").hide();
		$("#order_success").show();
	}
...
function printAgain(bno){
	window.location="/page/alipay/sinceorder/ship-store-print.html?orderNo="+bno;
}
	...
	if (url.indexOf("?") != -1) {
		...
		for (var i = 0; i < strs.length; i++) {
			if ([ strs[i].split("=")[0] ] == "orderNo") {
				params.bno = unescape($.trim(strs[i].split("=")[1]));
				$("#orderNo").html(params.bno);
			}
			if ([ strs[i].split("=")[0] ] == "status") {
                //对页面做填充
				params.status=unescape($.trim(strs[i].split("=")[1]));
