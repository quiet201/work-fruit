...
var yjFlag = '';
 ...
 //判断当前运单状态 1。电子运单  0.普通运单 默认为电子运单
    ...
    //var url ='../../tencent/range/ship_submit.html';
	...
	var OrderTimeListCookies = "OrderTimeListCookies";
	...
	function initNext(){
		...
		//&& sentAsbkId==''
		var yjSenderInfo = getCookie("yjSenderInfo");
		yjSenderInfo = $.parseJSON(yjSenderInfo);
		if(yjSenderInfo && yjSenderInfo != null && $.trim(yjSenderInfo) != "" && asbkId=='' ){
			yjFlag = '1';
			$("#senderNameLabel").text(yjSenderInfo.yj_senderNameLabel); 
			$("#senderName").val(yjSenderInfo.yj_senderNameLabel);
			$("#senderPhoneLabel").text(yjSenderInfo.yj_senderPhoneLabel); 
			$("#senderPhone").val(yjSenderInfo.yj_senderPhoneLabel);
			$("#senderAddress").text(yjSenderInfo.yj_senderAddress); 
			$("#senderDetailAddress").val(yjSenderInfo.yj_senderAddress);
			$("#senderCityCode").val(yjSenderInfo.yj_senderCityCode); 
			selectTheWaybill(yjSenderInfo.yj_senderCityCode);
		}else{
			deleteCookie("yjSenderInfo");
	...
	function getUrlParams() {
		...
		if (url.indexOf("?") != -1) {
			...
			strs = str.split("&");
			if ([ strs[0].split("=")[0] ] == "senderName") {
				yjFlag = '1';
				var senderNameTxt = unescape($.trim(strs[0].split("=")[1]));
				var senderPhoneTxt = unescape($.trim(strs[1].split("=")[1]));
				var senderAddressTxt = unescape($.trim(strs[2].split("=")[1]));
				var cityCode = unescape($.trim(strs[3].split("=")[1]));
				
				$("#senderNameLabel").text(senderNameTxt);
				$("#senderPhoneLabel").text(senderPhoneTxt); 	
				$("#senderAddress").text(senderAddressTxt);
				
				$("#senderName").val(senderNameTxt);				
				$("#senderPhone").val(senderPhoneTxt);
				$("#senderDetailAddress").val(senderAddressTxt);
				
				$("#senderCityCode").val(cityCode);
				setSenderHintInfo();
				selectTheWaybill(cityCode);
			}else{
				/*if ([ strs[0].split("=")[0] ] == "YJ") {
					alertError("您在近6个月内未寄件下单，请完善信息进行寄件下单");
				}*/
				initNext();
			}
...
//		 }
		...
		$.ajax({
			...
			success: function(json){
				if(json){
					...
					window.location = "/../../tencent/range/ship_submit.html?orderJson="+encodeURI(json);
				}
			},
			error:function(XMLHttpRequest, textStatus, errorThrown){
	...
	function toConfirmPage(){
		var orderJson =  packagingOrderData();
		window.location = "/../../tencent/range/ship_submit.html?orderJson="+encodeURI(orderJson);
	}
...
//		orderInfo["remarkLabel"] = remarkLabel;	//备注标签
		...
		if(yjFlag == '1'){
			var yjSenderInfo = {};
			yjSenderInfo["yj_senderNameLabel"] = $("#senderNameLabel").text(); 
			yjSenderInfo["yj_senderPhoneLabel"] = $("#senderPhoneLabel").text();
			yjSenderInfo["yj_senderAddress"] = $("#senderAddress").text();
			yjSenderInfo["yj_senderCityCode"] = $("#senderCityCode").val();

			setCookie("yjSenderInfo",JSON.stringify(yjSenderInfo));
		}
	}
...
//		setDtoParam(queryParam,"destAddr",destCode);
	...
	function toSelectAddressBook(){
		...
		location.href = "/page/tencent/addressbook/send/address_menber1.html?addressToPage=/page/tencent/order/ship.html&shipAskId="+shipAskId;
	}
			...
			success: function(result){
				if(result){
					if(result.code == "200"){
						...
						clearCookie();
						//跳转成功页面
						window.location = "/page/tencent/order/ship_success.html?resultJson="+JSON.stringify(result);
...
function searchProductFeeAndTime_bak(){
...
//		searchFeeAndTime(queryValue);
		...
		$.ajax({
			...
			success : function(result) {
				if(result && result.length>0){
					var buff = setProdectShowAddr(result,params.srcName);
					$("#div_id_prodect_show").empty().append(buff.join(""));
//					toggleChange();
//					$("#div_id_prodect_show").show();
					flag = true;
//					setTimeout(function(){$("#waitplease").hide();}, 1000); 
				}else {
					//var goodsWeigth = param.goodsWeigth;
					setTimeout(function(){
						$("#waitplease").hide();
						alertError('服务器开了会小差，请稍后重试！');
					}, 500); 
					flag = false;
				}
...
function setProudctType(obj,item){
	...
	$("#span_id_feed_show").show();
	$("#js_content_cplx").text(productName);
	$("#productType").val(item.productCode);
	if(0==price){
		 $("#yuguFright").text("暂无数据");	
	}else{
