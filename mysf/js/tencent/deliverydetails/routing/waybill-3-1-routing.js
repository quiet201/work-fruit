	...
	$.ajax({
				...
				success : function(data) {
					...
					if ($.trim(data) != "") {
						...
						var doBookSendTime=$.parseJSON(data).doBookSendTime;
						    isBind = $.parseJSON(data).IsBind;
						if('true'==isMessage){
							//$("#bookTime").removeAttr("click");
							$('#noMessage').hide();
							if('true'== noMessage){
								//此时后台未查到改派记录
								$('#isMessage').show();
							...
							if('1'==isBind){///如果是本人
								//false表示已经预约过，true表示没预约过
								if('true'==doBookSendTime){
									$('#bookTime').attr('class','ui-btn btn-submitserver');
									$("#bookTime").attr("href","/page/tencent/deliverydetails/bookTime/bookSendTime.html?bno="+bno);
								}
								else if('false'==doBookSendTime){
									$('#bookTime').attr('class','ui-btn btn-submitserver');
									$("#bookTime").text("查看预约派件");
									$("#bookTime").attr("href","/page/tencent/deliverydetails/bookTime/bookSendTime_success.html?bno="+bno);
								}
							}
							else if('1'!=isBind){
								$("#bookTime").attr("onClick",'alertError("不是收件人不能使用该服务")');					
							}
						}
						else{
							if('1'!=isBind){
								...
								$("#bookTime").attr("onClick",'alertError("服务持续开放中，敬请期待")');	
							}
							$('#noMessage').show();
							$('#isMessage').hide();
						}
			////////////预约派件时间/////////
						//doBookTime($.parseJSON(data).doBookSendTime);
						//alert($.parseJSON(data).doBookSendTime);	
						...
						if (values) {
							...
							if (values.employ) {
								...
								var xiaogeDs="/page/tencent/accepted/accepted.html?empName="+deliverName+"&mobile="+
						...
						//设置跳转链接
						$(".ui-angle-right").attr("href","/page/tencent/deliverydetails/routing/routeDetail.html?bno="+obj.bno);
					};
...
function hurryToSend(orderNo){	
	$.ajax({
		...
		data : null,
		dataType : "json",
		url : "/service/order/orderContro/hastenBill/"+orderNo+"/3",
...
//////////////////////////////////////////////////////////////手Q预约派件时间///////////////////////
function showBookTime(isBind,imgArea,bno){
	if("1"==isBind&&imgArea=='true'){
		$('#bookTime').attr('class','ui-btn btn-submitserver');
		$("#bookTime").attr("href","/page/tencent/deliverydetails/bookTime/bookSendTime.html?bno="+bno);
	}else if ("1"==isBind&&imgArea!='1') {
		alertError('服务持续开放中，敬请期待');
	}
	else{
		alertError('不是收件人不能使用该服务');
...
function doBookTime(doBookSendTime){
	if(doBookSendTime=='true'){
		$('#bookTime').attr('class','ui-btn btn-submitserver');
		$("#bookTime").attr("href","/page/tencent/deliverydetails/bookTime/bookSendTime.html?bno="+bno);
	}
}
