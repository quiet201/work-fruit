...
function init(){
	//listem();
	var addressObject=getCookie("questionnaireQQAddress");
	//这是是
...
function listem(){
	$("#checkedOne").unbind().bind("click",function(){
		...
		setCookieMin("questionnaireQQAddress",JSON.stringify(queryParam),60*24);
		getAddressDetail("/page/tencent/address/province.html?addrName=destName&addrCode=destCode&addrCountyName=dcountyName&addrCountyCode=dcountyCode&type=3&gatChild=-1");
	});
};
...
function getParams(){
	...
	var queryParam={
				sex : getCheckbox("sex"),
				age : getCheckbox("age"),
				provinceName : $("#fromprovince").val(),
				//provinceCode : $("#provinceCode").val(),
				cityName : $("#fromcity").val(),
				//cityCode : $("#cityCode").val(),
				countyName : $("#fromcounty").val(),
...
function setParams(data){
	...
	if (data) {
		...
		$("#fromprovince").val(data.provinceName);
		$("#fromcity").val(data.cityName);
		$("#fromcounty").val(data.countyName);
		$("#province").html(data.provinceName);
		$("#city").html(data.cityName);
		$("#county").html(data.countyName);
...
function Validate(){
	 ...
	 //3. 您所在的地区是
	 var province= $("#fromprovince").val();
	 var city = $("#fromcity").val();
	 var county = $("#fromcounty").val();
	 if(!variable1ObjectIsNull(province)){
		 location.href = "#dest_03";
		 alertErrors('请选择所在地区');
		 return ;
	 }
	 if("香港特区"!=province &&"台湾省"!=province && "澳门特区"!=province ){
		 if(!variable1ObjectIsNull(city) || !variable1ObjectIsNull(county)){
			 location.href = "#dest_03";
			 alertErrors('请选择所在地区');
			 return ;
		 }
...
function submit(){
	console.log("fromprovince"+$("input[name=fromprovince]").val());
	console.log("fromcity"+$("input[name=fromcity]").val());
	console.log("fromcounty"+$("input[name=fromcounty]").val());
	console.log("toprovince"+$("input[name=toprovince]").val());
	console.log("tocity"+$("input[name=tocity]").val());
	console.log("tocounty"+$("input[name=tocounty]").val());
	var queryParam=getParams();
	console.log(queryParam );
	setCookieMin("questionnaireQQAddress",JSON.stringify(queryParam),60*24);
	...
	questionnaireDto.province = questionnaireDto.province.replace('特区','').replace('区','').replace('省','').replace('市','');
	questionnaireDto.city =questionnaireDto.city.replace('辖县','').replace('辖区','');
	$('#submitFrom').hide();
	$('#submitFrom').unbind();//去掉a标签中的onclick事件
	$.ajax({
		type : "POST",
		data : questionnaireDto,
		url : "/service/tencent/questionnaire/saveQuestionnaire",
