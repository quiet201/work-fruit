...
$(document).on("ajaxBeforeSend",function(b){if(b["data"][1]["isLoding"]==false){return}var a=[];a.push('<div class="maskbox-loading">');a.push('<div class="cc">');a.push('<div class="loading">&nbsp;</div>');a.push("</div>");a.push("</div>");$(document.body).append(a.join(""))}).on("ajaxError",function(c,b,a,d){}).on("ajaxComplete",function(c,b,a){if(c["data"][1]["isLoding"]==false){return}$(".maskbox-loading").remove()});
window.JHope={};JHope.Tool={};(function(a){a.getUrlParam=function(){var b=window.location.hash;if(!b){return}b=b.substring(1);b=decodeURIComponent(unescape(decodeURI(b)));return JSON.parse(b)};a.setUrlParam=function(b){var d={};for(var c in b){if(!b[c]||b[c]=="null"){continue}d[c]=b[c]}return JSON.stringify(d)};a.forwardHref=function(c,b){window.location.href=c+"#"+encodeURI(escape(encodeURIComponent(a.setUrlParam(b))))}})(JHope.Tool);